...
# Date: 20 Jan 2025
# Version: 03
# Description: The script processes predicted climate data of the future period.
# Duration: Approximately 2 hours
# Saved files size: ~37.5 MB
...
future_folder <- "input/predicted_climate/future_period/" # Defining path to future climate data
files_in_folder <- list.files(path = future_folder, full.names = TRUE) # List all files in future folder
...
predicted_future_climate <- setNames(lapply( # Processing predicted future climate data
  ...
  FUN = function(combination) {
    ...
    future_start_year <- info$future_period[1] # Getting future start year
    future_end_year   <- info$future_period[2] # Getting future end year
      ...
      FUN = function(parameter) {
        ...
        relevant_files <- grep(
          pattern = paste0(parameter_abbreviation, "_EUR-11_", global_institute, "-", global_model, 
                           "_rcp85_r1i1p1_", regional_developer, "-", regional_model, 
        ...
        monthly_layers <- lapply( # Extracting monthly data and processing each month
          ...
          FUN = function(month) {
            ...
            valid_layers <- which( # Filtering layers based on month and future period
              layer_months == month &  # Filtering by months
                layer_years >= future_start_year & # Filtering by start of future period
                layer_years <= future_end_year # Filtering by end of future period
            )
...
output_folder <- "output/Task4" # Defining output folder path
if (!dir.exists(paths = output_folder)) dir.create(path = output_folder, recursive = TRUE) # Creating output folder
for (i in seq_along(predicted_future_climate)) {
  predicted_future_climate[[i]] <- readAll(predicted_future_climate[[i]]) # Ensuring all data is read into memory
}
save_file <- paste0(output_folder, "/predicted_future_climate.RData") # Defining output file path
save(predicted_future_climate, file = save_file) # Saving processed climate data
...
lapply(
  ...
  FUN = function(combination) {
    ...
    lapply(
      X = names(predicted_future_climate[[combination]]), # Creating maps for checking
      FUN = function(layer_name) {
        x = predicted_future_climate[[combination]][[layer_name]] # Getting layer name
        ...
        plot(
          x = predicted_future_climate[[combination]][[layer_name]], # Plotting the layer
